    ...
    return (
        <nav className='overflow-hidden select-none'>
            <div className='mx-auto max-w-screen-sw   w-full my-auto'>
                {/*Desktop*/}
                <div className="max-ssw:hidden py-4 flex  sw:px-32 lsw:px-14 px-8  justify-center w-full flex-row items-center ">
                    <div className='flex flex-row items-center gap-8'>
                        {items.map((i, index) =>
                            <div key={index} className='flex flex-col items-center relative justify-center h-11'>
                                ...
                                <div className={`${hovered === index ? 'opacity-100' : 'opacity-0 '} absolute transition-opacity duration-300 mx-auto bottom-0 rounded-full w-2 h-2 ${isInverted ? 'bg-black' : 'bg-white'}`}></div>
                            </div>
                        )}
