    ...
    return (
        <div className="flex flex-row max-md:flex-col justify-between mt-40 items-center gap-8">
            <div className={`flex flex-col ${reverse ? 'md:order-2' : ''}`}>
                <h2 className={`text-[64px] pb-4 max-ssw:text-[50px] text-left ${reverse ? 'md:text-right' : ''} leading-none font-berghan`}>{item.title}</h2>

                {item.text.split("\n").map((line, index) => (
                    // <p key={index}>{line}</p> // Wrap each line in <p> for spacing
                    <p key={index} className={`ssw:text-[20px] text-justify ${item.images.length > 1 ? ' max-w-[600px]' : 'w-full'} max-md:max-w-full mt-4`}>{line}</p>
                ))}
                {item.stacl && item.stacl.length > 0 ?
